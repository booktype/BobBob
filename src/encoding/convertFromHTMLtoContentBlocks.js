...
function getBlockDividerChunk(block: DraftBlockType, depth: number, attributes: Map, style: Map): Chunk {
  return {
    text: '\r',
    inlines: [OrderedSet()],
    entities: new Array(1),
...
function reverseStyleMap(){
  for (let styleName in DefaultDraftInlineStyle){
    let style = DefaultDraftInlineStyle[styleName].style
    let reverseStyle = {}
    for(let prop in style){
      reverseStyle[`${prop}__${style[prop]}`] = styleName
    }
  }
}
  ...
  if (styleToCheck) {
    ...
    currentStyle = currentStyle.withMutations(style => {
      ...
      for(let prop in properties){
        for (let styleName in DefaultDraftInlineStyle){
          let inlineStyle = DefaultDraftInlineStyle[styleName]
          if(inlineStyle.tag){
            if(htmlElement.type==inlineStyle.tag){
              style.add(styleName)
            }
          }
        }
...
function joinChunks(A: Chunk, B: Chunk): Chunk {
  const joined = joinChunksTest(A,B)
  return joined
}
function joinChunksTest(A: Chunk, B: Chunk): Chunk {
...
function genFragment(
  ...
  if (nodeName === '#text') {
    ...
    if(node.nextSibling){
      blocks.push({
        type: node.parentElement.nodeName.toLowerCase(),
        depth:depth+1,
        attributes:{},
        style: {}
      })
  ...
  if (
    ...
    var entkey
    if(!newEntityMap.size){
      entkey = 1
    }else{
      entkey = Number(newEntityMap.keySeq().last()) + 1
    }
...
function getChunkForHTML(
  ...
  // for(let block of safeBody.children){
  //   // unnestBlocks(block)
  // }
  if (!safeBody) {
    return null;
    ...
    contentBlocks: chunk.text.split('\r').map(
      (textBlock, ii) => {
        ...
        return new ContentBlock({
          ...
          data: Map({
            attributes: nullthrows(chunk).blocks[ii].attributes,
            style: nullthrows(chunk).blocks[ii].style
          }),
          text: textBlock,
          characterList,
        });
