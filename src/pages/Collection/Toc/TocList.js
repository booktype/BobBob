...
export default class TocList extends React.Component {
  ...
  static propTypes = {
    items: PropTypes.arrayOf(PropTypes.shape({_id: PropTypes.any.isRequired})),
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (isPropsUpdated) {
      ...
      this.setState({items: listWithChildren(itemsNew), dragItem: null, isDirty: false});
    }
  }
  ...
  collapse = (itemIds) => {
    ...
    if (itemIds == 'NONE') {
      ...
      this.setState({
        collapsedGroups: items.filter(item => item[childrenProp].length).map(item => item._id)
      });

    } else if (isArray(itemIds)) {
      const groups = items.filter(item => item[childrenProp].length && itemIds.indexOf(item._id)).map(item => item._id);

      this.setState({collapsedGroups: groups});
    }
  };
  ...
  tryIncreaseDepth(dragItem) {
    ...
    const pathFrom = this.getPathById(dragItem._id);
    ...
    if (itemIndex > 0 && newDepth <= maxDepth) {
      ...
      if (collapsedGroups.indexOf(prevSibling._id) == -1) {
  ...
  getSplicePath(path, options = {}) {
    ...
    path.forEach((index, i) => {
      if (i === lastIndex) {
        currentPath.$splice = [
          [
            index, numToRemove, ...itemsToInsert
          ]
        ];
      } else {
        const nextPath = {};
        currentPath[index] = {
          [options.childrenProp]: nextPath
        };
  ...
  getRealNextPath(prevPath, nextPath) {
    ...
    if (prevPath.length < nextPath.length) {
      ...
      if (nextPath[npLastIndex] > prevPath[npLastIndex]) {
        ...
        if (target[childrenProp] && target[childrenProp].length && collapsedGroups.indexOf(target._id) == -1) {
  ...
  onDragEnd = (e) => {
    ...
    this.setState({dragItem: null, isDirty: false});
  ...
  onMouseEnter = (e, item) => {
    ...
    const {dragItem} = this.state;
    if (dragItem._id === item._id)
      return;

    const pathFrom = this.getPathById(dragItem._id);
    const pathTo = this.getPathById(item._id);
  ...
  onToggleCollapse = (item) => {
    const {collapsedGroups} = this.state;
    const index = collapsedGroups.indexOf(item._id);

    if (index > -1) {
      this.setState({
        collapsedGroups: collapsedGroups.filter(id => id != item._id)
      });
    } else {
      this.setState({
        collapsedGroups: collapsedGroups.concat(item._id)
      });
    }
  };
  ...
  renderDragLayer() {
    ...
    const {dragItem} = this.state;
    const el = document.querySelector('.nestable-' + group + ' .nestable-item-' + dragItem._id);
    ...
    return (
      <div className="nestable-drag-layer">
        <ol className="nestable-list" style={listStyles}>
          <TocItem item={dragItem} options={options} isCopy/>
        </ol>
      </div>
    );
  }
    ...
    return (
      <div className={cn("nestable", "nestable-" + group, {'is-drag-active': dragItem})}>
        <ol className="nestable-list nestable-group">
          {items.map((item, i) => {
            return (<TocItem key={i} item={item} options={options}/>);
          })}
        </ol>
