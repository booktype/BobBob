...
import Icon from 'material-ui/svg-icons/hardware/keyboard-arrow-down';
class TocItem extends Component {
  render() {
    ...
    const isDragging = !isCopy && dragItem && dragItem._id === item._id;
    const hasChildren = item[childrenProp] && item[childrenProp].length > 0;
    const isCollapsed = collapsedGroups.indexOf(item._id) > -1;

    let Handler;

    let itemProps = {
      className: cn("nestable-item" + (isCopy
        ? '-copy'
        : ''), "nestable-item" + (isCopy
        ? '-copy'
        : '') + '-' + item._id, {'is-dragging': isDragging})
    };
    ...
    const collapseIcon = hasChildren
      ? (<Icon className={cn("nestable-item-icon", isCollapsed
        ? "icon-plus-gray"
        : "icon-minus-gray")} onClick={e => onToggleCollapse(item)}/>)
      : '';
        ...
        {hasChildren && !isCollapsed && (
          <ol className="nestable-list">
            {item[childrenProp].map((item, i) => {
              return (<TocItem key={i} item={item} options={options} isCopy={isCopy}/>);
            })}
          </ol>
        )}
      </li>
